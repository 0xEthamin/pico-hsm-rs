...
mod tests
{
    ...
    /// by Microchip's CryptoAuthLib for an identical input. The oracle
    /// was generated by `tools/checkmac-oracle/main.c`; see
    /// `docs/checkmac-validation.md` for the build procedure.
    ///
    ...
    {
        let mut slot_value = [0u8; HASH_LEN];
        for i in 0..HASH_LEN { slot_value[i] = i as u8; }
        let mut challenge = [0u8; HASH_LEN];
        for i in 0..HASH_LEN { challenge[i] = (i + 32) as u8; }
        let mut other_data = [0u8; 13];
        for i in 0..13 { other_data[i] = 0x80 + i as u8; }
        let mut serial = [0u8; 9];
        for i in 0..9 { serial[i] = 0xE0 + i as u8; }
